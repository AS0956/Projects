...
public class BasicKlondike implements KlondikeModel<BasicCard> {
  ...
  private List<BasicCard> drawPiles;
  ...
  public void startGame(List<BasicCard> deck, boolean shuffle, int numPiles, int numDraw) {
    ...
    for (int i = 0; i < numDraw && !workingDeck.isEmpty(); i++) {
  ...
  private boolean isOneRankLower(Card lower, Card higher) {
  ...
  public String toString() {
    ...
    for (int i = 0; i < foundationPiles.size(); i++) {
      Pile foundation = foundationPiles.get(i);
      sb.append(" ");
      if (foundation.isEmpty()) {
        sb.append("<empty>");
      } else {
        sb.append(foundation.peekTop().toString());
      }
    }
    sb.append("\n");
    ...
    if (drawPiles.isEmpty()) {
      ...
      for (int i = 0; i < numVisible; i++) {
        sb.append(" ").append(drawPiles.get(i).toString());
      }
    }
    sb.append("\n");

    // Cascade piles (multiple lines for rows)
    int maxHeight = 0;
    for (Pile pile : cascadePiles) {
    ...
    for (int row = 0; row < maxHeight; row++) {
      for (int pile = 0; pile < cascadePiles.size(); pile++) {
        Pile p = cascadePiles.get(pile);
        if (row < p.size()) {
          if (p.isCardFaceUp(row)) {
            sb.append(p.getCardAt(row).toString());
          } else {
            sb.append("?");
          }
        }
        if (pile < cascadePiles.size() - 1) {
          sb.append(" ");
        }
      }
      sb.append("\n");
