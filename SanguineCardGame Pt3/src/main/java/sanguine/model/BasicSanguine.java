...
public class BasicSanguine implements SanguineModel {
  ...
  private final List<ModelStatusListener> modelListeners = new ArrayList<>();
  ...
  public void pass() throws IllegalStateException {
    ...
    Pawn newCurrentPlayer = isRedPlayer ? Pawn.RED : Pawn.BLUE;
    notifyTurnChanged();
  ...
  private int getRowScore(int index, Player player) {
    checkGameStarted();
  ...
  public void beginGame() {
    if (!gameStarted) {
      throw new IllegalStateException("Must call startGame() before beginGame()!\n");
    }
    notifyTurnChanged();
  }
